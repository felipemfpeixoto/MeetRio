...
struct EventPageContent: View {
    ...
    var body: some View {
        ...
        // POSTHOG
        .onAppear{
            switch event.eventCategory.eventType {
            case .nightlife:
                PostHogSDK.shared.capture("ClicouEvento(Nightlife)")
            case .hostel:
                PostHogSDK.shared.capture("ClicouEvento(Hostel)")
            }
        }
    }
            ...
            if let startDateTime = event.dateDetails?.startDateTime,
               let endDateTime = event.dateDetails?.endDateTime{
                ...
                myEvent = CalendarEvent(
                    ...
                    latitude: event.address?.location.latitude ?? 0,
                    longitude: event.address?.location.longitude ?? 0,
