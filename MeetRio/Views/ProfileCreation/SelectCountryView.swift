                        ...
                        Task {
                            do {
                                ...
                                if let selectedImage {
                                    try await Fornecedor.shared.userVariable!.saveImage(image: selectedImage)
                                }
                                
                                // TODO: (2) Ver a forma correta de fazer isso
                                guard let hospede = Fornecedor.shared.userVariable as? Hospede else {
                                    isLoading = false
                                    return
                                }
                                hospede.country = selectedCountry
                                Fornecedor.shared.userVariable = hospede
                                
                                arbiuPrimeiraVez = false
                                didStartSignUpFlow = false
                                try await Fornecedor.shared.userVariable!.updateItem()
                                
                                isShowingFullScreenCover = false
                                isLoading = false
                                
                                // MARK: Função antiga, não entendi o que fazia
                                // postLoginSuccess()
...
#Preview {
    SelectCountryView( isShowingFullScreenCover: .constant(true), arbiuPrimeiraVez: .constant(true), didStartSignUpFlow: .constant(true), willLoad: .constant(false), selectedImage: nil)
}
