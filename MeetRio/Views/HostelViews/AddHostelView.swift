...
struct AddHostelView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            // TODO: (0) Com certeza isso não deveria estar aqui, mas sim dentro do init de AllHostels()
            Task {
                try await Fornecedor.shared.allHostels.getAllElements()
                allHostels = Fornecedor.shared.allHostels
            }
        }
    }
    ...
    var hostelsContainer: some View {
        VStack {
            ForEach(allHostels, id:\.self.id) { hostel in
                Button {
        ...
        .alert("Enter Password", isPresented: $showPasswordAlert, actions: {
            ...
            Button("Confirm") {
                if validatePassword(enteredPassword) {
                    ...
                    Task {
                        try await Fornecedor.shared.addHostel(selectedHostelID!)
                        dismiss()
                    }
