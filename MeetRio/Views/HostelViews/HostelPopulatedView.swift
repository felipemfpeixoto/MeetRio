...
// MARK: Os erros mais comuns aqui são de integração com o UserManager para acessar o hostel do usuário

struct HostelPopulatedView: View {
    let screenWidth: CGFloat = UIScreen.main.bounds.width
    let screenHeight: CGFloat = UIScreen.main.bounds.height
    
    @State var userHostel: Hostel?
...
//                }
        ...
        .onAppear {
            Task {
                if let user = Fornecedor.shared.userVariable as? Hospede {
                    userHostel = try await Hostel(id: user.hostel!)
                }
            }
        }
    }
    ...
    var headerContainer: some View {
        VStack {
            ...
            Text(userHostel?.name ?? "")
                .font(Font.custom("Bircolage Grotesque", size: 26))
                .fontWeight(.semibold)
                .foregroundStyle(.white)
            Text("\(userHostel?.addressDetails?.street ?? ""), \(userHostel?.addressDetails?.number ?? "") ")
                .foregroundStyle(.white)
    ...
    var descriptionContainer: some View {
        ZStack {
            if let description = userHostel?.description {
                ZStack {
                    Text(description)
                        .font(.system(size: 15))
    ...
    var servicesContainer: some View {
        ZStack {
            if let services = userHostel?.services, services.count > 0 {
    ...
    var eventsContainer: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack {
                    // TODO: (0) Aqui temos q fazer uma query de todos os eventos do hostel de acordo com seu ID
